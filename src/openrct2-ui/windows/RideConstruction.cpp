...
public:
    ...
    void UpdateWidgets()
    {
        ...
        if (GetRideTypeDescriptor(rideType).HasFlag(RIDE_TYPE_FLAG_HAS_ALTERNATE_TRACK))
        {
            if (_currentlyShowingBrakeOrBoosterSpeed)
            {
            ...
            for (int i = 0; i < currentRide->GetRideTypeDescriptor().AlternateTrackList.count; i++)
            {
                ...
                widgets[WIDX_TRACK_1 + i].image = ImageId(currentRide->GetRideTypeDescriptor().AlternateTrackList.list[i].icon);
            }
        }
