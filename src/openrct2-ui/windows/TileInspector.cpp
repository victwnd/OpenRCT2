        ...
        if (windowTileInspectorSelectedIndex != -1)
        {
            ...
            switch (tileElement->GetType())
            {
	            ...
	            auto poolObj = poolElement->GetPoolEntry();
                    ...
                    // Raise / lower label
                    screenCoords.y = windowPos.y + widgets[WIDX_POOL_SPINNER_HEIGHT].top;
                    DrawTextBasic(dpi, screenCoords, STR_TILE_INSPECTOR_BASE_HEIGHT_FULL, {}, { colours[1] });

                    // Current base height
                    screenCoords.x = windowPos.x + widgets[WIDX_POOL_SPINNER_HEIGHT].left + 3;
                    auto ft = Formatter();
                    ft.Add<int32_t>(tileElement->BaseHeight);
                    DrawTextBasic(dpi, screenCoords, STR_FORMAT_INTEGER, ft, { colours[1] });
	            break;
        ...
        do
        {
            ...
            switch (type)
            {
                ...
                case TileElementType::Banner:
                    ...
                    break;
                case TileElementType::Pool:
                    typeName = LanguageGetString(STR_POOL_TIP);//TODO make this different string
                    break;
                default:
...
private:
    ...
    void OnPrepareDraw() override
    {
        ...
        switch (tileElement->GetType())
        {
          ...
          case TileElementType::Pool:
                widgets[WIDX_POOL_SPINNER_HEIGHT].top = GBBT(propertiesAnchor, 0) + 3;
                widgets[WIDX_POOL_SPINNER_HEIGHT].bottom = GBBB(propertiesAnchor, 0) - 3;
                widgets[WIDX_POOL_SPINNER_HEIGHT_INCREASE].top = GBBT(propertiesAnchor, 0) + 4;
                widgets[WIDX_POOL_SPINNER_HEIGHT_INCREASE].bottom = GBBB(propertiesAnchor, 0) - 4;
                widgets[WIDX_POOL_SPINNER_HEIGHT_DECREASE].top = GBBT(propertiesAnchor, 0) + 4;
                widgets[WIDX_POOL_SPINNER_HEIGHT_DECREASE].bottom = GBBB(propertiesAnchor, 0) - 4;
                widgets[WIDX_POOL_CHECK_BROKEN].top = GBBT(propertiesAnchor, 1);
                widgets[WIDX_POOL_CHECK_BROKEN].bottom = GBBB(propertiesAnchor, 1);
                widgets[WIDX_POOL_CHECK_SLOPED].top = GBBT(propertiesAnchor, 2);
                widgets[WIDX_POOL_CHECK_SLOPED].bottom = GBBB(propertiesAnchor, 2);
                widgets[WIDX_POOL_CHECK_EDGE_N].top = GBBT(propertiesAnchor, 3) + 7 * 0;
                widgets[WIDX_POOL_CHECK_EDGE_N].bottom = widgets[WIDX_POOL_CHECK_EDGE_N].top + 13;
                widgets[WIDX_POOL_CHECK_EDGE_NE].top = GBBT(propertiesAnchor, 3) + 7 * 1;
                widgets[WIDX_POOL_CHECK_EDGE_NE].bottom = widgets[WIDX_POOL_CHECK_EDGE_NE].top + 13;
                widgets[WIDX_POOL_CHECK_EDGE_E].top = GBBT(propertiesAnchor, 3) + 7 * 2;
                widgets[WIDX_POOL_CHECK_EDGE_E].bottom = widgets[WIDX_POOL_CHECK_EDGE_E].top + 13;
                widgets[WIDX_POOL_CHECK_EDGE_SE].top = GBBT(propertiesAnchor, 3) + 7 * 3;
                widgets[WIDX_POOL_CHECK_EDGE_SE].bottom = widgets[WIDX_POOL_CHECK_EDGE_SE].top + 13;
                widgets[WIDX_POOL_CHECK_EDGE_S].top = GBBT(propertiesAnchor, 3) + 7 * 4;
                widgets[WIDX_POOL_CHECK_EDGE_S].bottom = widgets[WIDX_POOL_CHECK_EDGE_S].top + 13;
                widgets[WIDX_POOL_CHECK_EDGE_SW].top = GBBT(propertiesAnchor, 3) + 7 * 3;
                widgets[WIDX_POOL_CHECK_EDGE_SW].bottom = widgets[WIDX_POOL_CHECK_EDGE_SW].top + 13;
                widgets[WIDX_POOL_CHECK_EDGE_W].top = GBBT(propertiesAnchor, 3) + 7 * 2;
                widgets[WIDX_POOL_CHECK_EDGE_W].bottom = widgets[WIDX_POOL_CHECK_EDGE_W].top + 13;
                widgets[WIDX_POOL_CHECK_EDGE_NW].top = GBBT(propertiesAnchor, 3) + 7 * 1;
                widgets[WIDX_POOL_CHECK_EDGE_NW].bottom = widgets[WIDX_POOL_CHECK_EDGE_NW].top + 13;
...
*/
                SetCheckboxValue(
                 WIDX_POOL_CHECK_EDGE_NE, tileElement->AsPool()->GetEdges() & (1 << ((0 - GetCurrentRotation()) & 3)));
                SetCheckboxValue(
                 WIDX_POOL_CHECK_EDGE_SE, tileElement->AsPool()->GetEdges() & (1 << ((1 - GetCurrentRotation()) & 3)));
                SetCheckboxValue(
                 WIDX_POOL_CHECK_EDGE_SW, tileElement->AsPool()->GetEdges() & (1 << ((2 - GetCurrentRotation()) & 3)));
                SetCheckboxValue(
                 WIDX_POOL_CHECK_EDGE_NW, tileElement->AsPool()->GetEdges() & (1 << ((3 - GetCurrentRotation()) & 3)));
                SetCheckboxValue(
                 WIDX_POOL_CHECK_EDGE_E, tileElement->AsPool()->GetCorners() & (1 << ((0 - GetCurrentRotation()) & 3)));
                SetCheckboxValue(
                 WIDX_POOL_CHECK_EDGE_S, tileElement->AsPool()->GetCorners() & (1 << ((1 - GetCurrentRotation()) & 3)));
                SetCheckboxValue(
