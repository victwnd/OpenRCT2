...
validate_global_widx(WC_TOP_TOOLBAR, WIDX_POOL);
...
static Widget window_top_toolbar_widgets[] = {
    ...
    MakeRemapWidget({ 30, 0}, {30, TOP_TOOLBAR_HEIGHT + 1}, WindowWidgetType::TrnBtn, WindowColour::Tertiary   ,SPR_TOOLBAR_WATER,          STR_BUILD_FOOTPATH_TIP              ), // Pools
...
private:
    ...
    {
        if (WindowFindByClass(WindowClass::Pool) == nullptr)
        {
            ContextOpenWindow(WindowClass::Pool);
        }
        else
        {
            ToolCancel();
            WindowCloseByClass(WindowClass::Pool);
        }
    }
            ...
            if (!gSceneryCtrlPressed)
            {
                if (InputTestPlaceObjectModifier(PLACE_OBJECT_MODIFIER_COPY_Z))
                {
                    ...
                    constexpr auto flag = EnumsToFlags(
                        ...
                        ViewportInteractionItem::LargeScenery, ViewportInteractionItem::Pool);
...
public:
    ...
    void OnDropdown(WidgetIndex widgetIndex, int32_t selectedIndex) override
    {
        switch (widgetIndex)
        {
            case WIDX_FILE_MENU:
                ...
                // New game is only available in the normal game. Skip one position to avoid incorrect mappings in the menus of
                // the other modes.
                if (gScreenFlags & (SCREEN_FLAGS_SCENARIO_EDITOR))
                    selectedIndex += 1;

                // Quicksave is only available in the normal game. Skip one position to avoid incorrect mappings in the menus of
                // the other modes.
    ...
    void OnPrepareDraw() override
    {
        ...
        widgets[WIDX_POOL].type = WindowWidgetType::TrnBtn;
        ...
        if (gScreenFlags & SCREEN_FLAGS_EDITOR)
        {
            ...
            if (gEditorStep != EditorStep::LandscapeEditor && gEditorStep != EditorStep::RollercoasterDesigner)
            {
                ...
                widgets[WIDX_POOL].type = WindowWidgetType::Empty;
        ...
        if (WindowFindByClass(WindowClass::Pool) == nullptr)
            pressed_widgets &= ~(1ULL << WIDX_POOL);
        else
            pressed_widgets |= (1ULL << WIDX_POOL);
